...
# < Script parameters > (adjust them for each run)
setwd("Analysis/")
clusters_list <- list.files('Metapatients/with_entire_count_matrix/final_clusters/')
...
run_DEAnalysis_for_dis_metapatients <- function(dis,clusters_filename,results_dir){
  ...
  for(k in 1:n_clusters){
    ...
    write.table(for_jon,file=paste('Metapatients/Ranked_metapatients',"/",dis,"_DEGs_",metap_method,n_clusters,"_",k,".rnk",sep=""),sep="\t",col.names=F,row.names=F, quote=FALSE)
...
for (dis in dis_list){
  # dis <- 'Schizophrenia'
  print(dis)
  cclustes_name <- clusters_list[grep(paste(dis,"_normalized",sep=""),clusters_list)]
  if(length(cclustes_name) == 0){
    cclustes_name <- clusters_list[grep(paste(dis,"_combat",sep=""),clusters_list)]
  }
  print(cclustes_name)
  clusters_filename = paste('Metapatients/with_entire_count_matrix/final_clusters/',cclustes_name,sep="")
    
  # Preparing output directory
  results_dir = paste("Metapatients/with_entire_count_matrix/DEAnalysis/",dis,sep="")
