...
# tsne(as.data.frame(assays(se.filt)$logCPM), labels=se.filt$type, perplex=40, seed=TRUE)  + ggtitle("tsne based on logCPM")
# tsne(as.data.frame(assays(se.filt)$logCPM), labels=se.filt$series, perplex=40, seed=TRUE, text=as.character(se.filt$type)) + ggtitle("tsne based on logCPM")
# 
# tsne(as.data.frame(assays(se.filt)$logCPM), labels=se.filt$type, perplex=94, seed=TRUE)  + ggtitle("tsne based on logCPM")
# tsne(as.data.frame(assays(se.filt)$logCPM), labels=se.filt$series, perplex=94, seed=TRUE, text=as.character(se.filt$type)) + ggtitle("tsne based on logCPM")

# tsne(as.data.frame(assays(se.filt)$logCPM), labels=se.filt$series, scale=3, text=as.character(se.filt$type))

# par(mfrow=c(1, 1))
# tsne(as.data.frame(assays(se.filt)$logCPM), labels=se.filt$tissue, seed=TRUE, text=as.character(se.filt$simplified_type)) + ggtitle("tsne based on logCPM perplex2")
...
#   x
# }, batch, outcome)
# plot(sampleDendrogram, main="Hierarchical clustering of samples - type")
# 
# # Coloured by series and type as text
# logCPM <- cpm(dge.filt.norm, log=TRUE, prior.count=3)
# d <- as.dist(1-cor(logCPM, method="spearman"))
# sampleClustering <- hclust(d)
# batch <- as.integer(factor(se.filt$series))
# sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
# names(batch) <- colnames(se.filt)
# outcome <- as.character(se.filt$type)
# names(outcome) <- colnames(se.filt)
...
#   x
# }, batch, outcome)
# plot(sampleDendrogram, main="Hierarchical clustering of samples - coloured by series")
# legend("topright", paste("Batch", sort(unique(batch)), levels(factor(se.filt$series))), fill=sort(unique(batch)))
# 
# # Colour: tissue | text: type
# logCPM <- cpm(dge.filt.norm, log=TRUE, prior.count=3)
# d <- as.dist(1-cor(logCPM, method="spearman"))
# sampleClustering <- hclust(d)
# batch <- as.integer(factor(se.filt$tissue))
# sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
# names(batch) <- colnames(se.filt)
# outcome <- as.character(se.filt$type)
# names(outcome) <- colnames(se.filt)
...
#   x
# }, batch, outcome)
# plot(sampleDendrogram, main="Hierarchical clustering of samples - coloured by tissue")
# legend("topright", paste("Batch", sort(unique(batch)), levels(factor(se.filt$tissue))), fill=sort(unique(batch)))
##### ------------------------------------------------------------

## NEW hierarchichal clustering
...
# OLD
# colors=c("orange","blue")
# dendog <- as.dendrogram(pvclust_result)
# nodePar <- list(lab.cex = 0.6, pch = c(NA, 19), cex = 0.7, col = colors[metadata$type])
# plot(dendog, nodePar=nodePar, leaflab= "none", ylab="height")
# # plot(dendog, nodePar=nodePar, labels= FALSE, ylab="height")
# plot(dendog, type="fan", tip.color=colors[metadata$type], leaflab= "none", ylab="height")
# pvrect(pvclust_result)
...
if (length(unique(se$series)) > 1){
  ...
  # sampleClustering <- hclust(d)
  # batch <- as.integer(factor(se$series))
  # sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
  # names(batch) <- colnames(se.filt)
  # outcome <- as.character(se.filt$type)
  # #outcome <- paste(as.character(se.filt$condition), as.character(se.filt$type), sep="-")
  # names(outcome) <- colnames(se.filt)
  # sampleDendrogram <- dendrapply(sampleDendrogram, function(x, batch, labels) {
  #   if (is.leaf(x)) {
  #     attr(x, "nodePar") <- list(lab.col=as.vector(batch[attr(x, "label")]))
  ...
  #   x
  # }, batch, outcome)
  # plot(sampleDendrogram, main="Hierarchical clustering of samples after Combat")
  # legend("topright", paste("Batch", sort(unique(batch)), levels(factor(se$series))), fill=sort(unique(batch)))
}
...
if (length(unique(se$series)) > 1){
  ...
  # sampleClustering <- hclust(d)
  # batch <- as.integer(factor(series))
  # sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
  # names(batch) <- colnames(se.filt)
  # outcome <- as.character(se.filt$type)
  # #outcome <- paste(as.character(se.filt$condition), as.character(se.filt$type), sep="-")
  ...
  #   x
  # }, batch, outcome)
  # plot(sampleDendrogram, main="Hierarchical clustering of samples after QR Decomposition")
  # legend("topright", paste("Batch", sort(unique(batch)), levels(factor(se$series))), fill=sort(unique(batch)))
}
...
#   x
# }, batch, outcome)
# plot(sampleDendrogram, main="Hierarchical clustering of samples after SVD batch effect removal")
# legend("topright", paste("Batch", sort(unique(batch)), levels(factor(se$series))), fill=sort(unique(batch)))
...
# }, batch, outcome)
# plot(sampleDendrogram, main="Hierarchical clustering of samples after SVD batch effect removal")
# legend("topright", paste("Batch", sort(unique(batch)), levels(factor(se.filt$type))), fill=sort(unique(batch)))
